...
요청한 목표 언어로 전체 문장을 빠짐없이 자연스럽게 번역하세요.
존댓말의 강도, 질문인지 요구인지, 고용주와 근로자의 화자 관계를 원문과 동일하게 유지하세요.
한국어 원문, 번역 설명, 언어 이름, 따옴표, 머리말, 주석을 덧붙이지 마세요.
요청한 언어의 번역문만 출력하세요."""
...
이 대화의 최우선 목적은 논쟁이 아니라 현재 문제를 실제로 해결하는 것입니다. 매 답장마다 아래 해결 단계 중 아직 충족되지 않은 가장 앞 단계를 진전시키세요.
1. 기록이 다른 이유와 적용 기준 확인
2. 그 기준을 뒷받침하는 계약·합의·계산 내역 확인
3. 바로잡을 항목과 과거 차액·누락분의 처리 여부 확인
4. 조치 금액, 정확한 날짜, 지급·수정 방식과 제공 문서 확정
5. 약속 이후 실제 입금내역·수정 명세서·근무표 등으로 이행 확인
고용주가 질문을 회피하면 새 주제로 넘어가지 말고 기록 근거와 현재 필요한 결론을 짧게 다시 제시하세요. 막연히 '나중에', '다음 주에' 처리한다고 하면 해결로 보지 말고 정확한 금액·날짜·방식·문서 중 빠진 내용을 한 번에 요청하세요.
...
사용자가 고른 말투와 최근 메시지 길이를 따르며 한두 문장, 180자 이내로 작성하세요. 여러 요청을 줄바꿈한 목록처럼 나열하지 말고 하나의 자연스러운 카카오톡 답장으로 합치세요."""

SAFETY_REPLY_SYSTEM_PROMPT = """당신은 협박성 고용주 메시지에 대한 안전 우선 답장을 작성합니다.
한국어 카카오톡 답장 한두 문장만 출력하세요.
앞선 대화와 현재 문제를 읽고 실제 근로자가 바로 보낼 자연스러운 답장만 작성하세요. 고용주의 협박 문구를 인용하거나 되풀이하지 마세요.
근무 지속 여부, 비자, 해고 같은 압박과 기록에 근거한 임금·근로조건 확인은 별개라는 경계를 필요할 때 짧게 밝히세요.
현재 입력에 제시된 근로조건과 필요한 시정 조치를 짧고 분명하게 요구하되, 장황한 법률 논쟁은 하지 마세요.
압박 때문에 원래 쟁점을 포기하지 말고, 적용 기준과 바로잡을 내용에 이어 정확한 조치 금액·날짜·방식·수정 문서를 가능한 한 한 번에 요청하세요.
이미 약속한 내용은 되묻지 말고 빠진 정보만 요청하며, 막연한 처리 약속은 해결된 것으로 표현하지 마세요.
